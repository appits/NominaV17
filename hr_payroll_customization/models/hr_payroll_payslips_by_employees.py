...
class HrPayslipEmployees(models.TransientModel):
    ...
    def compute_sheet(self):
        ...
        ##############################################################
        # struct_date = payslips.is_first_second_monthly_fortnight(self.structure_id)
        # payslip_run.date_start = struct_date.get('date_from')
        # payslip_run.date_end = struct_date.get('date_to')
        # days = payslip_run._get_monday_saturday_sunday_numbers()
        # payslip_run.monday_numbers = days.get('monday_numbers')
        # payslip_run.saturdays_sunday_numbers = days.get('saturdays_sunday_numbers')
        ##############################################################
        ...
        for contract in self._filter_contracts(contracts):
            ...
            values.update( self._set_payslip_vals(payslip_run, contract) )
    ...
    def _set_payslip_vals(self, run, contract):
        val = {
            'rate_id': run.rate_id.id,
            'rate_amount': run.rate_amount,
            'monday_numbers': run.monday_numbers,
            'saturdays_sunday_numbers': run.saturdays_sunday_numbers,
            'biweekly_advance': run.biweekly_advance,
        }
