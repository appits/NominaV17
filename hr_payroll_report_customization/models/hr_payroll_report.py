...
class HrPayrollReport(models.Model):
    ...
    def _get_codes(self):
        return {
            'add': "'BASIC','ALW','BASIC2','BASIC3', 'BASIC4', 'BASIC5', 'UTIL'",
            'ded': "'DED', 'DED2', 'DED3', 'DED4', 'COMP','CONTRIB', 'Descpresta'",
            'general': "'NET','BASIC','ALW','BASIC2','BASIC3', 'BASIC4', 'BASIC5', 'DED', 'DED2', 'DED3', 'DED4', 'COMP','CONTRIB', 'UTIL', 'Descpresta'",
        }


    def _new_select(self, codes):
        #TO INHERIT
        return ""


    def _new_where(self, codes):
        #TO INHERIT
        return ""


    def _new_group_by(self, codes):
        #TO INHERIT
        return ""
    ...
    def init(self):
        ...
        new_select = self._new_select(codes)
        new_where = self._new_where(codes)
        new_groupby = self._new_group_by(codes)
        query = """
            SELECT
                p.id as id,
                ...
                CASE WHEN src.code in ({add}) and wd.id = min_id.min_line THEN pl.total ELSE 0 END as count_assignments,
                CASE WHEN src.code in ({ded}) and wd.id = min_id.min_line THEN pl.total ELSE 0 END as count_deductions,
                {sel}
            ...
            FROM
                (SELECT * FROM hr_payslip WHERE state IN ('verify', 'done', 'paid')) p
                    ...
                    left join hr_payslip_line pl on (pl.slip_id = p.id and pl.category_id in (select src.id from hr_salary_rule_category src where src.code IN ({general})))
            ...
            GROUP BY
                ...
                """.format(add=codes["add"], ded=codes["ded"], general=codes["general"], sel=new_select, whe=new_where, group=new_groupby)
